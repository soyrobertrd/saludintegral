...
export default function Services() {
  const services = [
    ...
    },
    {
      ...
      img: "https://images.unsplash.com/photo-1584820927498-cfe5211fd8bf?auto=format&fit=crop&q=80&w=800",
          ...
          {services.map((service, idx) => (
            ...
            >
              ...
              <div className="w-full lg:w-1/2">
                <div className="aspect-video rounded-3xl overflow-hidden shadow-sm shadow-primary/5 relative bg-surface-container-low border border-outline-variant/5">
                   <img 
                    src={service.img} 
                    alt={service.title} 
              ...
              <div className="w-full lg:w-1/2 space-y-8">
                <div className="w-16 h-16 bg-primary/5 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110">
                  {service.icon}
                </div>
                <h2 className="text-3xl md:text-5xl font-black font-headline tracking-tighter group-hover:text-primary transition-colors">
                  {service.title}
                </h2>
                ...
                <ul className="space-y-4 pt-4">
                  {service.features.map((feature, fidx) => (
                    <li key={fidx} className="flex items-center gap-4 text-sm font-bold text-on-surface-variant/70 uppercase tracking-widest">
                       <Shield className="text-primary" size={16} /> {feature}
                    </li>
                  ))}
                </ul>
                <div className="pt-8">
                   <Link to="/contact" className="bg-on-surface text-white px-10 py-5 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-primary transition-all inline-flex items-center gap-4 shadow-xl shadow-on-surface/10">
