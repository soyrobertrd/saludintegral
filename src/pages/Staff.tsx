  ...
  return (
    <div className="bg-background min-h-screen pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-8">
          ...
          <div className="flex flex-wrap gap-3 w-full md:w-auto">
            {staffCategories.map((cat, i) => (
              <button 
                key={i}
                className={`px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all border ${i === 0 ? 'bg-primary text-on-primary border-primary' : 'bg-surface-container-low text-on-surface-variant hover:border-primary border-outline-variant/10'}`}
              >
                {cat}
              </button>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {staff.map((person, idx) => (
            <motion.div 
               ...
               className="group"
            >
              <div className="aspect-3/4 rounded-3xl bg-surface-container-low overflow-hidden mb-8 relative border border-outline-variant/5 shadow-sm group">
                 <img 
                    src={person.img} 
                    alt={person.name} 
                 ...
                 <div className="absolute inset-x-6 bottom-6 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                    <div className="flex gap-2">
                       <Link to={`/staff/${person.id}`} className="grow bg-primary px-6 py-4 rounded-xl flex justify-between items-center text-white shadow-xl">
                          <span className="text-[9px] font-black uppercase tracking-widest">Ver Perfil Completo</span>
                          <ArrowRight size={16} />
                       </Link>
                       <button className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-primary shadow-lg border border-outline-variant/10">
                          <Mail size={18} />
                       </button>
                    </div>
                 </div>
              ...
              <div className="space-y-4">
                 ...
                 <div className="flex flex-col gap-2 pl-11">
                    <span className="text-[10px] font-black uppercase tracking-widest text-primary opacity-60 leading-none">{person.role}</span>
                    <div className="flex items-center gap-2 text-[10px] font-bold text-on-surface-variant/50 uppercase tracking-widest">
                       <GraduationCap size={14} className="opacity-40" /> {person.edu}
                    </div>
                 </div>
              </div>
            </motion.div>
        ...
        {/* Inquiry Section */}
        <div className="mt-32 p-12 md:p-20 bg-primary/5 rounded-3xl border border-primary/10 flex flex-col md:flex-row items-center justify-between gap-12 text-center md:text-left overflow-hidden relative">
           <div className="absolute top-0 right-0 w-96 h-96 bg-primary/10 blur-[130px] rounded-full translate-x-1/2 -translate-y-1/2"></div>
           <div>
              <h2 className="text-3xl md:text-5xl font-black font-headline tracking-tighter mb-4">¿No encuentra su especialista?</h2>
              <p className="text-lg text-on-surface-variant font-body opacity-70">Nuestro equipo de conserjería le ayudará a encontrar el personal adecuado.</p>
           </div>
           <Link to="/contact" className="bg-primary text-on-primary px-12 py-6 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-primary-container transition-all shadow-2xl shadow-primary/30 flex items-center gap-4">
              <MessageCircle size={20} /> Solicitar Asistencia
           </Link>
        </div>
      </div>
    </div>
